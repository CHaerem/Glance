...
app.get("/", async (_req, res) => {
	// Read the simple UI HTML file and serve it directly
	try {
		const htmlContent = await fs.readFile(path.join(__dirname, '..', 'simple-ui.html'), 'utf8');
		res.send(htmlContent);
	} catch (error) {
		console.error('Error serving simple UI:', error);
