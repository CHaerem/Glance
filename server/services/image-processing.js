...
class ImageProcessingService {
    ...
    findClosestSpectraColor(r, g, b) {
        ...
        let closestColor = this.paletteLab[1]; // Default to white

        for (const color of this.paletteLab) {
    ...
    applyDithering(imageData, width, height, algorithm = 'floyd-steinberg', saturationBoost = 1.3) {
        ...
        let ditheredData = saturationBoost > 1.0
     ...
     */
    async createTextImage(text, targetWidth = 1200, targetHeight = 1600) {
        try {
            const svg = `
