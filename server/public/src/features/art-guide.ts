...
import type { CurrentDisplay, GuideResponse, Hint, HintPools } from '../types';

// Guide state
let guideSending = false;
const guideSessionId = 'web-' + Date.now();
let responseAutoHideTimer: ReturnType<typeof setTimeout> | null = null;
let guideDrawerInitialized = false;
let discoverySuggestionsInitialized = false;

// Current display info for context hints
let currentDisplayInfo: CurrentDisplay | null = null;

// Hint pools for dynamic suggestions
const hintPools: HintPools = {
  // Time-based hints
  morning: [
 ...
 */
async function handleSearchOrGuide(query: string): Promise<void> {
 ...
 */
function showInlineResponse(text: string, isLoading = false): void {
 ...
 */
function hideInlineResponse(): void {
  const responseDiv = document.getElementById('guideResponse');
  if (responseDiv) {
 ...
 */
function autoHideResponse(delay = 5000): void {
  if (responseAutoHideTimer) {
    clearTimeout(responseAutoHideTimer);
  }
 ...
 */
async function sendGuideMessage(message: string): Promise<void> {
  ...
  try {
    ...
    if (hasResults && typeof window.displayGuideResults === 'function') {
      ...
      if (playlistLabel && playlistName) {
        ...
        if (refreshBtn) (refreshBtn as HTMLElement).style.display = 'none';
      }
 ...
 */
async function refreshCurrentDisplay(): Promise<void> {
  try {
 ...
 */
function pickRandom<T>(arr: T[], count = 1): T | T[] {
 ...
 */
async function fetchCurrentDisplayInfo(): Promise<void> {
  try {
 ...
 */
function generateDynamicHints(): Hint[] {
  const hints: Hint[] = [];
  ...
  const timeHint = pickRandom(hintPools[timeOfDay]) as Hint;
  hints.push(timeHint);

  // Add a mood or subject hint (alternate randomly)
  if (Math.random() > 0.5) {
    hints.push(pickRandom(hintPools.moods) as Hint);
  } else {
    hints.push(pickRandom(hintPools.subjects) as Hint);
  }
  ...
  if (currentDisplayInfo && currentDisplayInfo.title && Math.random() > 0.6) {
    ...
    hints[2] = pickRandom(hintPools.context) as Hint;
  }
 ...
 */
function renderSmartHints(): void {
  ...
  container.querySelectorAll('.hint-link').forEach((link) => {
    link.addEventListener('click', (e) => {
      ...
      (document.getElementById('searchInput') as HTMLInputElement | null)?.blur();

      const element = link as HTMLElement;
      if (element.dataset.action) {
        handleHintAction(element.dataset.action);
      } else if (element.dataset.query) {
        sendGuideMessage(element.dataset.query);
      }
    });
  });
}
 ...
 */
function setupDiscoverySuggestions(): void {
 ...
 */
function refreshDiscoveryHints(): void {
  fetchCurrentDisplayInfo().then(() => {
    renderSmartHints();
  });
}
 ...
 */
async function handleHintAction(action: string): Promise<void> {
  switch (action) {
    case 'surprise':
      await surpriseMe();
      break;
