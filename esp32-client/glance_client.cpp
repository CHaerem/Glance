...
void setup() {
  ...
  if (connectToWiFi()) {
    ...
    sendLogToServer("Device started - WiFi connected to " + String(WIFI_SSID_VAR), "INFO");
    updateDeviceStatus();
    
    // Fetch and display new image
    if (fetchAndDisplayImage()) {
      Serial.println("Image updated successfully");
      sendLogToServer("Image updated successfully", "INFO");
      deviceStatus.lastUpdateSuccess = true;
    } else {
      Serial.println("Image update failed");
      sendLogToServer("Image update failed: " + deviceStatus.lastError, "ERROR");
    ...
    // Note: Can't send log since WiFi failed
...
bool reportDeviceStatus() {
  // Report status to local server
  HTTPClient http;
  http.begin(STATUS_URL_VAR);
  http.addHeader("Content-Type", "application/json");
  http.addHeader("User-Agent", "Glance-ESP32/1.0");
  
  // Create simple status payload for local server
  DynamicJsonDocument doc(1024);
  doc["deviceId"] = DEVICE_ID_VAR;
  ...
  Serial.println("Sending status to local server:");
  ...
  Serial.printf("Local server response: %d\n", httpResponseCode);
  if (httpResponseCode != 200) {
    Serial.println("Response: " + response);
  }
  
  return httpResponseCode == 200; // Local server returns 200 on success
}
...
void sendLogToServer(String message, String level) {
  if (WiFi.status() != WL_CONNECTED) {
    return; // Can't send logs without WiFi
  }
  
  HTTPClient http;
  String logUrl = String(API_BASE_URL_VAR) + "logs";
  http.begin(logUrl);
  http.addHeader("Content-Type", "application/json");
  ...
  // Don't log the response to avoid infinite loops
  if (httpResponseCode != 200) {
    Serial.printf("Log send failed: %d\n", httpResponseCode);
  }
}

void enterDeepSleep(unsigned long duration) {
  Serial.println("Preparing for deep sleep...");
  sendLogToServer("Entering deep sleep for " + String(duration / 1000000) + " seconds", "INFO");
